...
def build(root: Path, source: Path, output: Path | None = None):
    ...
    if project:
        ...
        project_url = urlsplit(project.get('url', ''))
        project_parts = unquote(project_url.path).strip('/').split('/')
        if (project_url.scheme != 'https' or project_url.hostname != 'linear.app'
                or len(project_parts) < 3 or project_parts[:2] != [workspace, 'project']
                or not project_parts[2]):
            raise WorkbenchError('Project URL does not belong to the declared workspace')
