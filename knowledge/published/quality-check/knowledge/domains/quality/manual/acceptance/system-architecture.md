...
- [系统架构概览](#系统架构概览)
- [系统上下文与事实边界](#系统上下文与事实边界)
- [进程内组件与职责](#进程内组件与职责)
- [当前实现链路](#当前实现链路ratio-分配预览)
- [目标执行链路](#目标链路结论执行与状态回查)
- [跨层数据结构与转换](#跨层数据结构与转换)
- [当前能力与目标能力对账](#当前能力与目标能力对账)
- [目标前端状态边界](#目标前端状态边界)

## 系统架构概览

**事实边界：** 验收系统同时读取本平台的管理数据和外部任务系统的最终状态。本平台数据库可以保存交付任务、派生快照、预览和操作记录，但只有外部任务系统能证明 task/clip 是否真实完成分配、通过或打回。

**软件责任：** 在进程内部，API 维护接口契约，Service 组织完整业务用例，纯 Python 算法负责确定性计算，Repository 和外部客户端分别处理两类 I/O。当前分层的价值来自这些责任和数据边界，而不是 Router、Service、Repository 等名称本身。

**当前范围：** 当前代码只贯通查询与 Ratio 数量预览；正式分配、规则结论、通过/打回和回查仍属于目标设计。

## 系统上下文与事实边界

**上下文关系：** 用户通过工作台和 API 进入 Python 应用；应用分别访问本平台数据库、人员数据和外部任务系统，刷新任务把外部事实重建为平台读模型。
...
**协作原则：** HTTP 接入、业务编排、纯计算、领域数据访问和外部调用各自维护一类责任；实线表示当前代码，虚线表示目标能力。
...
**调用结果：** 这条链路只生成并保存数量预览，不选择具体任务，也不改变外部任务状态。
...
**目标结果：** 结论执行必须在调用外部系统前重取指标、重算结论和反查任务，并在即时响应后继续回查最终状态。
...
**转换目的：** 接口对象、算法对象、Repository 行和数据库字段分别服务不同边界，转换用于隔离变化而不是增加名词层次。
...
**对账结论：** 当前纵向切片可以用于理解和继续开发查询、展开与数量预览；分配、结论、执行和刷新仍不能按已实现能力使用。
...
**交互要求：** 目标验收工作台不能只显示“成功或失败”，至少要区分 `idle`、`loading`、`ready`、`stale`、`executing` 和 `refreshing`。统计结果携带 `computed_at`；执行后先显示即时摘要，再等待外部状态回查和数据刷新。无数据、部分失败、重复点击和重试都需要明确反馈。

**现实边界：** 这些状态来自目标前端设计，不是当前页面已经完整实现或部署的证据。目标人员模型也仍出现旧 `level`、`unit_price` 字段，与当前人员历史设计存在冲突；实现前需要回到真实接口和 Schema 裁决。
