...
兼容信号不能只抄成“兼容通过”。把它作为一个可回答的问题，至少追到：**旧输入/旧状态是什么、当前代码怎样恢复或合并、用户最终看到什么、失败时怎样降级、哪个长期 owner 维护这项行为**。若兼容行为由公共组件实现，即使本次没有修改公共组件源码，也要让公共能力页承载机制和边界；领域页只说明本领域怎样消费它。验证报告只证明结果，源码、契约或聚焦测试负责解释机制。
