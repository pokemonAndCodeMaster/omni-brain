...
| 验证项 | 实际结果 | 状态 |
|---|---|---|
| Python 自动化测试 | ConfigManager、分析目录与查询校验、HTTP 参数边界归一化等共 13 项通过 | 通过 |
...
| Vue 组件回归 | 表格、范围筛选、全页筛选事件、指标聚合、总览项目拆分、双轴序列、任务汇总表与页面级 Ref 解包回归共 17 项通过 | 通过 |
| Vue 类型与构建 | `vue-tsc` 通过，Vite 生产构建通过 | 通过（有 bundle 大小警告） |
| HTTP 数据链 | health、rows、scene 聚合和前端路由均可真实访问 | 通过 |
| V2 指标目录与任务查询 | `GET /api/manual-qc/analysis/catalog` 返回 5 个基础维度和首批统计指标；`POST /query` 在服务端完成任务跨日期聚合、比例筛选和排序 | 通过 |
| V2 动态问题选项 | `POST /facets` 返回问题标签和选项；指定 `驾驶行为分类 / CUT_IN` 后可按“占 Bad 比例 > 20%”筛出唯一任务 | 通过 |
| V2 任务根表 | 16 条员工日快照汇总为 4 个跨周期任务；城区交互任务-A 标注提交 447、Good 占比 75.3915%；页面组件按任务信息、标注、验收进度、验收结果分组展示 | 通过（组件测试、生产构建与真实 Chromium 路由渲染） |
| V2 表头与筛选交互 | 四个业务分组分别跨 2、2、4、1 个实际子列；任务和项目同时支持文字包含及精确多选；筛选层挂载到页面根节点，在左侧文本列和横向滚动后的最右数值列均完整显示；输入“泊车”后任务根表由 4 条缩为 1 条 | 通过（17 项组件测试与 1440×900 真实 Chromium 交互） |
| V2 页面空白回归 | 真实路由加载后，任务表实际收到数组、布尔状态、总数和指标目录；页面显示“按标注任务比较产出与验收”及 4 条任务，无未处理 Vue 渲染异常 | 通过 |
...
比率，并提供按天趋势和 Bad 问题排行。当前任务根表支持全部首批业务列的本地筛选、
排序和列调整；旧 V1 明细组件保留了逐级懒加载实现，但没有挂载为当前 V2 用户路径。
总览与统计卡片均可编辑、拖动、缩放和保存，统计卡片可将数量与比率放在双轴上。

平台还不包含采样配额、精确任务预览、人工确认、执行、部分失败和状态回查；
也尚未实现任务→日期→组→标注员下钻、Good/Bad 与问题标签详情侧栏、验收覆盖矩阵和
问题标签—选项专用明细。这些边界与实施顺序见
