...
const option = computed<EChartsOption>(() => {
  ...
  for (const row of props.rows) {
    ...
    current.assigned += row.total_accept_assigned
    current.completed += row.total_accept_completed
    current.passed += row.total_accept_passed
    current.rejected += row.total_accept_rejected
