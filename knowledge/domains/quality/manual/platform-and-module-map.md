...
title: 人工质检平台与模块地图
description: 根据 Batch 1 目标架构说明人工质检组件怎样协作，并区分目标、历史和当前原型。
...
目标平台把用户交互、接口契约、业务编排、领域计算、数据访问和外部任务状态分给不同模块。这样的分工只有在职责、输入输出和事实边界清楚时才有价值，不能用 Router、Service、Repository 等类名代替软件结构解释。

固定代码只落地任务查询、日期展开和 Ratio 数量预览这一条纵向切片。需求接纳、验收监控、返工跟踪、统一人员工作台和外部执行仍缺少完整产品或实现证据。
...
Batch 1 没有给出可用的需求接纳/交付中心、验收监控分析、返工跟踪和统一人员工作台信息架构。K0 不为它们虚构页面；Batch 2 到达后再判断怎样补入而不破坏现有知识结构。
