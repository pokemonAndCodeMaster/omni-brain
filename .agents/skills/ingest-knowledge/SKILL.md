...
用户或交接已经给出父 commit 时必须传入 `--parent-commit`。工作台验证祖先关系并返回固定 diff 的变化路径、共享依赖和兼容信号；这些只是定向规划提示，不替代源码阅读或业务判断。来源页最终必须写入工作台冻结的实际来源路径、branch 和 HEAD commit，并保留本次比较使用的父 commit；detached HEAD 明确写成 detached，不伪造分支。不能一边引用新报告，一边继续把旧工作树或旧分支写成当前来源。

工作台还会从明确的常量变化和报告中的“由 A 增至 B”提取事实迁移。最终 `review` 会在候选中查找仍带“当前、最多、上限、通过、拒绝”等语义的旧数值；命中后必须更新为新事实，或明确标注它只属于哪个历史版本。这个扫描只处理高置信数字迁移，不能代替人工复核公式、状态和文字结论。
...
兼容信号不能只抄成“兼容通过”。把它作为一个可回答的问题，至少追到：**旧输入/旧状态是什么、当前代码怎样恢复或合并、用户最终看到什么、失败时怎样降级、哪个长期 owner 维护这项行为**。`identity-set` 返回的 `compatibility_probes` 给出定向搜索词；兼容问题必须登记至少一项实际命中这些机制词的源码或聚焦测试，不能只登记验证报告。若兼容行为由公共组件实现，即使本次没有修改公共组件源码，也要让公共能力页承载机制和边界；领域页只说明本领域怎样消费它。验证报告只证明结果，源码、契约或聚焦测试负责解释机制。
