...
> 状态：**已确认；切片一已实现并通过数据库、HTTP、组件测试与生产构建验证**
>
...
| 方案 | 优点 | 主要问题 | 结论 |
|---|---|---|---|
| 继续把快照行拉到浏览器统计 | 改动最少 | 当前有 1000 行上限；跨周期任务筛选不完整；动态问题标签容易漏算；多个卡片重复取数 | 不采用 |
...
**2026-07-26 实际结果：** 16 条员工日快照汇总为 4 个跨周期任务；通过率低于 80% 的
任务返回“园区泊车任务-D”“高速变道任务-B”；`驾驶行为分类 / CUT_IN` 占 Bad 比例高于
20% 时只返回“城区交互任务-A”。详细命令与边界见
...
但当前问题选项的验收分配、完成、通过和打回都是 0。实施**切片二**前需要扩充固定种子：
...
切片一已完成。后续实施模型应只进入**切片二**，不得同时重写总览卡片和图表编辑器。开始前先：

1. 从任务根节点读取现有受控查询结果，不重新引入浏览器端快照聚合；
2. 实现任务 → 日期 → 组 → 标注员下钻，并把子查询约束为父级路径；
3. 实现 Good/Bad、问题标签和问题选项的详情侧栏；
4. 完成“固定一个问题选项为列、筛选、排序”的真实页面路径；
5. 汇报实际修改、设计取舍、运行结果和仍然缺失的能力。

切片二通过后，才能进入总览卡片 V2；不能因为最终设计已经写明，就一次性实现全部 V2。
