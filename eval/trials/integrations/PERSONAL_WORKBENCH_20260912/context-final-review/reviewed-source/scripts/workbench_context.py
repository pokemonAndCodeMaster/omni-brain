...
def build(root: Path, source: Path, output: Path | None = None):
    ...
    if url.hostname != 'linear.app' or unquote(url.path).strip('/').split('/')[:3] != [workspace, 'issue', issue['id']]:
    ...
    for doc in docs:
        ...
        seen.add(doc['id'])
        if not doc_identity(doc.get('url', ''), workspace):
            raise WorkbenchError('Document belongs to a different workspace')
    required = {d['id'] for d in issue.get('documents', [])}
    urls = list(markdown_links(issue['description']))
    # Plain pasted URLs are also direct material references. Do not treat URLs
    # inside code samples as required background documents.
    for token in markdown_parser().parse(issue['description']):
