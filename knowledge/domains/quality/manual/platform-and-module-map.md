...
Batch 2 的平台边界新增交付中心、人员与权限、统一数据工作台和可配置看板。它们共享 Shell、表格、卡片、选择、预览、反馈、权限和可访问性语言，但不合并人工质检、大模型质检、自动化质检和专题质量各自的业务模型、表或流程。
...
父版本 Batch 1 没有给出可用的需求接纳/交付中心、验收监控分析、返工跟踪和统一人员工作台信息架构。当前候选已将交付任务与行动项、人员与权限、统一数据工作台及其导航接入长期知识；这补的是业务边界和目标设计，不是已部署页面、API、SSO 或端到端运行证据。

统一数据工作台的目标边界是由 `DataWorkbench` 管布局与事件、由 `useDataWorkbench` 管列/筛选/排序/选择/展开状态；业务页提供列契约、数据源、权限、批量动作和展开维度。跨页危险操作必须表达为明确 ID 或筛选快照加排除项，并遵循选择、预览、冻结集合、权限/版本复核、执行、回查、审计；这些仍是目标设计，不是当前实现证据。
