...
V1 固定统计不再作为不可编辑的特殊区域。切片四达到功能一致后，已经把它们转换为
系统预设图表卡片：
...
项目中只保留一套当前挂载的 V2 总览、图表和任务表实现，不让 V1 页面组件长期变成
第二套运行路径。总览、图表和表格的持久化入口仍然独立，只有统一保存交互被真实需求
确认后才合并。

**2026-07-26 实际结果与取舍：** 删除了不再挂载的 V1 固定统计图和日期—项目表，
同时移除页面测试中的旧组件 stub；V1 配置结构和旧四级聚合 API 仍承担迁移、回退和
顶部范围数据职责，因此保留。`useMetricWorkspace` 与 `useDashboardWorkspace` 分别
对应“保存总览”和“保存看板”两个当前用户动作，本轮只消除重复死代码，不在没有统一
保存交互设计的情况下强行合并。真实 Chromium 重载后显示 5 张图表、24 个任务，
首个任务展开得到 14 个日期；图表编辑预览正常，控制台无异常。自动化与生产构建全通过。
...
切片一至五已完成。后续实施模型不得继续以“清理或通用化”为下一产品切片，应先回到
真实验收工作选择一个可观察结果：

1. 若优先判断分配是否充分，设计任务 × 组/人员/Good/Bad/问题选项的验收覆盖矩阵；
2. 若优先判断配额是否执行到位，设计预期分配与实际分配的对照和缺口定位；
3. 若进入操作闭环，从采样预览、人工确认、任务执行、部分失败和状态回查中选择最薄闭环；
4. 新切片继续复用当前指标目录、受控查询、卡片和任务表，不新建平行统计口径；
5. Windows Edge 延迟最小化回弹继续单独取证；没有捕获到主动聚焦事件前，不把防御性
   焦点修改声明为根因修复。
