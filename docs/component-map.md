...
| `AgentRegistry` | 启动时读取并缓存已发布 Harness 的能力、默认执行器与可选执行器 | 保存 Run 或在请求期扫描全部历史 |
| `AgentRunRepository` | 在 PostgreSQL 保存 Run 元数据、聚合计数和追加式规范事件 | 启动进程、解释业务结果或保存工作区文件 |
| `AgentRunService` | 通过统一 Adapter 串行调度 Codex/OpenCode Run，衔接 worktree、进程、事件与最终状态 | SQL、模型评判、远程 Worker 或容器隔离 |
| `CodexExecutor` | 组装 `codex exec --json`、解析 JSONL、结构化结果和 stderr 诊断 | Run 持久化或业务状态迁移 |
| `OpenCodeExecutor` | 组装 `opencode run`、解析 JSON 事件并保留具体失败原因 | Run 持久化或 worktree 生命周期 |
| `CollaborationRepository` | 保存 Idea、Requirement、Revision、Thread Entry 与 Decision，并派生分页时间线 | 启动模型或复制 Run trace |
| `CollaborationService` | 执行业务状态规则、固定 admin、动作到能力映射和最低上下文快照 | SQL、自动接纳需求或通用工作流编排 |
...
| `AgentCatalogPage.vue` | 展示 Agent 摘要、Codex/OpenCode 健康和聚合 Run 计数，按需读取启动配置 | 不读取 Run trace，不轮询历史详情 |
| `AgentRunsPage.vue` | 展示轻量 Run 列表，并在选中后组合详情和增量 trace | 不在列表接口读取 prompt、result 或原始事件 |
| `IdeaInboxPage / IdeaDetailPage` | 记录原始 Idea，并在详情中组合人的消息、Agent 动作、时间线和转需求预览 | 列表不读取正文、Run 结果或 trace |
| `RequirementListPage / RequirementDetailPage` | 管理 candidate/accepted Requirement、Revision 和人工评审 | 不实现 Work、排期或自动接纳 |

## Agent Runtime 功能

| 组件或 composable | 单一职责 | 输入 / 输出 |
|---|---|---|
| `useAgentCatalog` | 首次并行读取 Agent 摘要与 Codex/OpenCode 健康，显式刷新注册表 | 输出目录、健康、加载和错误；无后台轮询 |
| `AgentLaunchDialog.vue` | 编辑一次任务的执行器、标题、prompt 和可选模型 | 接收单个 Agent 详情；发出 typed `submit/close` |
...
| 组件或 composable | 单一职责 | 输入 / 输出 |
|---|---|---|
| `useIdeaInbox / useRequirementList` | 读取分页摘要、筛选并执行显式创建 | 不读取正文、时间线或 trace |
| `useIdeaDetail / useRequirementDetail` | 进入明确对象后并行读取详情、Revision 元数据和时间线 | 不后台轮询；动作完成后显式刷新 |
| `CollaborationTimeline.vue` | 呈现 Entry、Run、Revision、Decision 的派生时间线 | 只有点击 Run 或使用候选时才读取 Run 详情 |
| `AgentActionPanel.vue` | 从业务对象发起白名单动作并显式选择 Codex/OpenCode | 不允许前端自定义 agent_id |
| `RequirementEditor.vue` | 编辑结构化 R1 并通过 `v-model` 发出完整新值 | 不直接调用 API，不覆盖历史 Revision |
| `RequirementDecisionPanel.vue` | 收集 accept/reject/defer/merge/reopen 的必填证据 | S0 不提供 NOW；不替人作决定 |
