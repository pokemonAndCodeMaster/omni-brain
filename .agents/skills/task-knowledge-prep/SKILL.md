...
对每个关键问题调用 `knowledge-query`，并按需读取文档、代码、Schema、配置、运行事实或请求人工确认。把查询范围、实际模式和动作写入事件，把来源、Claim 与冲突写入证据；保留查询 Skill 返回的缺口类型，需要专家决定时补充 `human_confirmation`。

不得用模型常识填补关键事实。需要正式摄入时，先说明影响并取得用户授权，再调用 `knowledge-ingest`；不要绕过它的来源、冲突和审核门禁。

当前回答关键问题时先用 `python scripts/task_case.py evidence <case-id> --kind <类型> --summary <摘要> --source <来源>` 取得 Evidence ID，再把该问题的 `status` 改为 `answered`、写入 `evidence_ids`，追加 `frame_updated` 事件并运行门禁。这个手工回写步骤是待验证基线，不得假装已有原子命令。
