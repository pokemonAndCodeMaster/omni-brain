...
> 状态：**已确认；切片一至四已实现，并通过数据库、HTTP、组件、生产构建和真实浏览器用户路径验证**
>
...
它们都必须指定问题标签和问题选项。切片四已经沿用相同参数化方式公开以下验收指标：
...
**2026-07-26 实际结果：** 页面使用 4 张 V2 系统预设和 1 张从 V1 迁移的用户卡片。
在真实 Chromium 页面中把“验收分配与完成”横轴从任务改为日期，新增验收通过率面积
图层，把分配量按项目拆成两条序列，并给通过率图层增加“项目 = 园区”筛选；实时预览
返回 14 个日期和 5 条序列。应用、保存并刷新后完整恢复；复制卡片、恢复原系统预设、
删除副本并再次保存均成功。编辑器滚动 700px 后右侧真实聚合预览位置保持不变。
最终数据库保存 5 张 `dashboard-v2` 卡片，不含 V1 `query/style` 字段。
...
切片一至四已完成。后续实施模型只进入**切片五：收束和回归**：

1. 先用源码引用和真实页面确认 V2 已经覆盖的 V1 实现，不删除仍承担回退职责的代码；
2. 删除不再挂载的固定统计和已被 V2 替代的浏览器聚合、类型与编辑器；
3. 评估两个 workspace 的重复状态逻辑，只有能保持三种呈现边界时才合并；
4. 运行全量自动化、生产构建和“查询 → 编辑 → 保存 → 刷新 → 下钻”的真实页面回归；
5. 单独记录 Windows Edge 延迟最小化回弹问题；没有捕获到主动聚焦事件前，不把防御性
   焦点修改声明为根因修复。
