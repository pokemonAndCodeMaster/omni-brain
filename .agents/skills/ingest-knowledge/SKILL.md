...
- **宽范围完整整理（`complete`）**：用户交来一个领域、模块或混合材料包，希望从零建立知识，或把新增材料有机融入现有知识。先审视本次材料地图和读后发现，再对照已有知识规划目录。不要在读材料前猜主题或现实身份。
...
`update/merge` 不是在旧页末尾追加“本批材料”“本次增量”或日期章节。写作前先通读原页标题和正文，把新发现放进原有语义结构；原结构承载不了时，只新增面向长期读者的稳定标题。批次、文件和摄入轮次只进入来源追溯与审查页。产品视图也应原位重排导航，不新增“增量导航”。写完后从头复读整页，合并重复的概述、来源边界和结论，使读者看到一篇连续知识，而不是两次摄入的拼接痕迹。
...
对于 `update/merge`，必须通读并重组最终整页，而不是只读文件尾部后追加内容；产品视图同样检查是否出现重复章节、重复边界或按摄入轮次分区。
