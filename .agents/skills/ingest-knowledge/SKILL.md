...
影响复核发现知识单元类型或落点不对、且该问题尚未登记来源/知识时，可用同一 `plan-unit` ID 提交完整新定义，工作台会更新计划；不用换 ID 叠加一个重复 owner。已经进入取源或写作后再暴露规划错误时，保留现有证据并只增加确有职责的单元。
...
一次优先只读当前 packet。优先连接：产品入口与用户动作 → Schema/类型与状态 → Service/算法 → Repository/SQL/迁移 → 前端转换与共享组件 → 测试和真实运行。固定 diff 或报告明确点名、但 packet 尚未返回的冻结文件，可以直接定向读取并在 `record` 登记；一次最多 12 项，不得借此递归搜索或无界通读。文档帮助定位，当前行为最终以固定源码、契约和运行结果为准。取得最低充分证据后立即写知识并 `record`；剩余候选不会改变答案时用 `--close-candidates` 停止，不扩展成整仓阅读。
