...
- [验收业务闭环](#验收业务闭环)
- [关键业务约束](#关键业务约束)
- [历史、目标与当前实现](#历史目标与当前实现)

## 生命周期概览

**核心判断：** 验收并不是“抽一些任务检查，然后点通过或打回”。它连接了标注结果、抽样作业、质量判断、外部状态变化和最终交付，任何一段没有闭合，都可能出现“表面完成、实际无法交付”。
...
- 现有材料能说明完整目标闭环和部分历史做法，当前代码只覆盖范围查询与 Ratio 数量预览。

**交付关系：** 交付任务是闭环的主对象：通过/打回之后还要确认可交付 Good 数量；标注员、验收员和执行者的产物不能混为一个状态。交付阶段、健康状态、行动项状态和验收结论也必须分别呈现，不能用单一标签替代四个维度。

## 业务阶段与核心对象

**阶段划分：** 标注、验收和执行分别产生不同结果。先分清三个阶段，再区分候选总体、样本、记录、结论和执行结果五类对象。
...
**流程范围：** 本节从标注结果可用开始，追踪选择、验收、结论、执行、回查、返工和交付汇总；表格补足每个环节的输入、输出和进入条件。
...
- **计划与实际必须并列**：需求目标时间、预估量、数据就绪时间和负责人等计划基线不能被实际到数时间、实际数量或后续调整覆盖。

## 历史、目标与当前实现

| 现实形态 | 当前知识能确认的内容 | 不能据此推断 |
|---|---|---|
