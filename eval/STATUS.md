...
- “验收分配缺口”跨层用例已完成修订参考与 Luna 盲测。弱模型沿 Catalog、Repository、HTTP、Vue 映射和任务表完成纵切，自主发现 `10 + 5` 最大组合并取得 `18/20`；独立数据库、API 和 Edge 证明四级结果 `173 → 26 → 12 → 3`、筛选、排序和下钻可用。候选还用正式知识和运行事实反证了首轮专家参考的类别互抵口径，参考已从 `d5a5fa6` 修正为 `35948ae`。开发 Harness 因此在同一质检领域的三类开发切片内验证，下一主线转入代码到知识的安全回写。
- “代码变化到知识回写”已形成固定输入、24 项内容题、`24/24` 强参考和四轮 Luna 盲测。第四轮候选 `3a6c5da` 达到工作台 `publish_ready`，完整保留 18 个快照字段和固定验证证据，并完成独立系统身份、父知识原位融合和产品视图同步；独立内容审计为 `22/24`，前端行转换与固定运行结果内化深度不足，关键题未全过。通用修正已进入 Harness `ebd696c`，完整回归通过但尚待下一真实切片重放。
- 相邻的“本地快照导入代码回写”已完成两轮 Luna 重放。第二轮由 `17/24` 提升到 `20/24`，证明提交祖先判断、外部契约冻结、软件责任闭合和固定运行结果内化的修正有效；旧版本句子、可复制 CLI、数据库公共能力和固定日期夹具边界仍缺。Harness `a54abcd` 只收束这些跨用例成立的完成标准；M4 保持 `implemented / human review required`。
...
| **本地快照导入代码回写** | 已发布质检知识 + 快照导入实现/固定报告 | 强参考与 24 项冻结问题 | Luna 两轮；`17/24 → 20/24` | 通用修正取得实质收益，四项关键语义仍缺；不发布 | 停止同题调优；由新复合开发工况反证通用完成标准 |
...
**当前结果：** [第二轮审计](trials/real_work/CODE_TO_KNOWLEDGE_WRITEBACK_SNAPSHOT_IMPORT_CODEX_LUNA_HARNESS_002/audit.md)为 `20/24`，比首轮 `17/24` 明显改善。弱模型正确判断提交祖先关系，完整吸收 11 列 CSV、18 列快照返回、13 键指标状态、导入模型、锁/事务和代表性运行结果，并更新既有软件结构页。

**未通过边界：** 来源页仍残留与当前提交冲突的旧句子；知识没有保留项目根可复制的 preview/apply 命令；数据库公共能力页漏掉这个新消费者；固定日期平移没有与产品能力分开。Harness `a54abcd` 已把“旧版本当前态复核、可运行入口、受影响公共能力、夹具变换边界”收束为通用完成标准，未再运行同题。完整回写仍为 `implemented / human review required`。
...
1. 跨模型、助手和邻近领域迁移（旧 `E7`）。

场景设计保存在 [`docs/experiments/knowledge-driven-work-experiments.md`](../docs/experiments/knowledge-driven-work-experiments.md)，但其动态执行状态必须回到本页。
...
| `f29328b`—`a54abcd` / `experiment/development-harness-v1` | 相邻快照导入回写与通用完成标准 | 第二轮由 `17/24` 提升到 `20/24`；当时的完整回归通过，M4 仍需人工审查 |
...
M1 同批优化和发布已经收口。M2 已在质检中文知识问答切片内验证，M3 已在页面、SQL 和跨层算法三类质检开发切片验证。M4 的两个代码回写切片证明隔离、契约、证据、父知识融合和视图同步具有价值，但关键语义仍会遗漏。下一轮不再分别追加同类题，而是固定一个新的开发复合工况，验证可信任务上下文、真实代码、项目运行和知识变化候选能否一次闭环。
