...
const columns = computed<ColumnDef<AggregateNode, unknown>[]>(() => [
  ...
  columnHelper.accessor('good_annotation_submitted', {
    id: 'good_annotation_submitted',
    header: 'Good 提交',
    size: 105,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('bad_annotation_submitted', {
    id: 'bad_annotation_submitted',
    header: 'Bad 提交',
    size: 105,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('total_accept_assigned', {
    id: 'total_accept_assigned',
    header: '验收分配',
    size: 105,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('total_accept_completed', {
    id: 'total_accept_completed',
    header: '验收完成',
    size: 105,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('total_accept_passed', {
    id: 'total_accept_passed',
    header: '验收通过',
    size: 105,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('total_accept_rejected', {
    id: 'total_accept_rejected',
    header: '验收打回',
    size: 105,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('good_accept_completed', {
    id: 'good_accept_completed',
    header: 'Good 完成',
    size: 108,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('good_accept_passed', {
    id: 'good_accept_passed',
    header: 'Good 通过',
    size: 108,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('good_accept_rejected', {
    id: 'good_accept_rejected',
    header: 'Good 打回',
    size: 108,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('bad_accept_completed', {
    id: 'bad_accept_completed',
    header: 'Bad 完成',
    size: 108,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('bad_accept_passed', {
    id: 'bad_accept_passed',
    header: 'Bad 通过',
    size: 108,
    cell: (context) => countCell(context.getValue()),
  }),
  columnHelper.accessor('bad_accept_rejected', {
...
<template>
  <section class="snapshot-table-section">
    ...
    <DataWorkbench
      :rows="rows"
      :columns="columns"
      :can-expand="(row) => row.level !== 'employee'"
      empty-text="当前筛选没有验收快照。"
      @row-expand="emit('expand', $event)"
    />
  </section>
</template>
